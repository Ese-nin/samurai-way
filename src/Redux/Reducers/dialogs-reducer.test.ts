...
test('newMessageText should be changed', () => {
    const startState = {
        dialogs: [
            {id: "1", name: "Leonid"},
            {id: "2", name: "Victoria"},
    ...
    const endState1 = dialogsReducer(startState, textareaMessageChangeAC(message1))
    const endState2 = dialogsReducer(startState, textareaMessageChangeAC(message2))

    expect(endState1.newMessageText).toBe(message1)
    expect(endState2.newMessageText).toBe(message2)
});
...
test('message should be added to array', () => {
    ...
    expect(endState1.newMessageText).toBe('')
    expect(endState2.newMessageText).toBe('')
